...
static func store_journey_context(settings: Dictionary, site: Dictionary) -> void:
	...
	match String(site.get("class", "")):
		...
		"dwarfhold":
			...
			# Overland arrivals walked in through the mountain mouth: the
			# hold spawns them at its own south gate, not the Great Hall,
			# so stepping through the stone reads as one continuous walk.
			settings["dwarfhold_scene_overland_arrival"] = true
